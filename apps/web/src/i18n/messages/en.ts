export const en = {
  brand: "Obscura",
  brandMarkLabel: "Obscura brand mark",
  brandTitle: "Image Editing Details",
  heroTitle: "Create privately. Export in seconds.",
  heroSubtitle:
    "Import local images or short clips, make focused edits, and download clean results. Files stay in this browser.",
  addMedia: "Add media",
  importMedia: "Import Media",
  chooseMedia: "Choose media files",
  previousAsset: "Previous asset",
  nextAsset: "Next asset",
  language: "Language",
  english: "English",
  chinese: "中文",
  privacy: "No media leaves this browser.",
  localOnly: "local only",
  localAdvantage: "Local & private",
  workspaceSections: "Workspace sections",
  library: "Media",
  localStorage: "Local Storage",
  noAsset: "No asset",
  preview: "Preview",
  edit: "Edit",
  effects: "Effects",
  layers: "Layers",
  output: "Output",
  export: "Export",
  processing: "Processing",
  processingQueue: "Processing queue",
  backgroundTasks: "Background tasks",
  tasks: "tasks",
  jobIdle: "Idle",
  jobQueued: "Queued",
  jobLoading: "Loading",
  jobProcessing: "Processing",
  jobCompleted: "Done",
  jobCanceled: "Canceled",
  jobFailed: "Failed",
  clearTask: "Clear task",
  clearCompletedTasks: "Clear completed",
  cancelTask: "Cancel task",
  retryTask: "Retry task",
  openResult: "Open result",
  newResult: "New result",
  addedToQueue: "Added to background queue.",
  assets: "assets",
  filterMediaType: "Filter media type",
  all: "All",
  image: "Image",
  video: "Video",
  noMediaYet: "No media yet",
  emptyLibrary: "Import files to start editing.",
  remove: "Remove",
  generatedAsset: "Generated",
  privateDefault: "Local workspace",
  emptyPreviewTitle: "Start Your Creation",
  emptyPreviewBody:
    "Drag and drop your images or videos here, or click to browse your local workspace.",
  emptyCapabilityQuality: "4K support",
  emptyCapabilityRaw: "RAW photos",
  emptyCapabilityFormats: "MP4, MOV, WEBM",
  compareOriginal: "Compare original",
  showEdited: "Show edited",
  compareView: "Compare view",
  compareOn: "Compare On",
  originalVersion: "Original",
  editedVersion: "Edited",
  previewTools: "Preview tools",
  zoomIn: "Zoom in",
  zoomOut: "Zoom out",
  fitToScreen: "Fit to Screen",
  fullscreenPreview: "Fullscreen preview",
  exitFullscreenPreview: "Exit fullscreen preview",
  selectAssetToEdit: "Select a media asset to reveal image or video tools.",
  cropRotate: "Crop & Rotate",
  cropPreset: "Crop preset",
  original: "Original",
  square: "1:1 Square",
  portrait: "4:5 Portrait",
  story: "9:16 Story",
  wide: "16:9 Wide",
  custom: "Custom",
  landscape: "16:9 Landscape",
  undoImageEdit: "Undo image edit",
  redoImageEdit: "Redo image edit",
  undoShort: "Undo",
  redoShort: "Redo",
  reset: "Reset",
  rotateMinus: "Rotate -90",
  rotatePlus: "Rotate 90",
  flipHorizontal: "Flip horizontal",
  flipVertical: "Flip vertical",
  outputWidth: "Output width",
  adjustments: "Beautify",
  brightness: "Brightness",
  contrast: "Contrast",
  saturation: "Saturation",
  filters: "Filters",
  filterStrength: "Filter strength",
  filterNone: "None",
  filterVivid: "Vivid",
  filterWarm: "Warm",
  filterCool: "Cool",
  filterMono: "Mono",
  filterFilm: "Film",
  filterFade: "Fade",
  resetBeautify: "Reset beautify",
  resetTransform: "Reset transform",
  resetLayers: "Reset layers",
  previewCropResult: "Preview crop result",
  watermarkText: "Watermark text",
  watermarkPosition: "Watermark position",
  watermarkOpacity: "Watermark opacity",
  watermarkOpacityPercent: "Opacity percent",
  watermarkImage: "Watermark image",
  chooseWatermarkImage: "Choose image",
  noWatermarkImage: "No image selected",
  watermarkImageHelper: "Image watermark replaces the visible text watermark until cleared.",
  clearWatermarkImage: "Clear watermark image",
  watermarkTopLeft: "Top left",
  watermarkTopRight: "Top right",
  watermarkBottomLeft: "Bottom left",
  watermarkBottomRight: "Bottom right",
  watermarkCenter: "Center",
  annotations: "Annotations",
  addTextAnnotation: "Add text",
  addRectangleAnnotation: "Add rectangle",
  addArrowAnnotation: "Add arrow",
  addBrushAnnotation: "Add brush",
  annotationText: "Text note",
  annotationRectangle: "Rectangle",
  annotationArrow: "Arrow",
  annotationBrush: "Brush stroke",
  noAnnotations: "No annotations yet",
  removeAnnotation: "Remove annotation",
  optional: "Optional",
  imageEditHelper:
    "Image edits are previewed locally and applied to a temporary export blob only when you prepare a download.",
  imageEditControls: "Image edit controls",
  autoEnhance: "Auto-Enhance",
  autoEnhanceDetail: "Local adjustment helper planned",
  backgroundRemover: "Background Remover",
  backgroundRemoverDetail: "Run the local model and add the transparent PNG as a new asset.",
  backgroundRemovalAction: "Remove background",
  backgroundRemovalRunning: "Removing background...",
  backgroundRemovalTask: "Background removal - transparent PNG",
  backgroundRemovalComplete: "Background removed.",
  backgroundRemovalFailed: "Background removal failed.",
  backgroundRemovalRetry: "Retry background removal",
  trimRange: "Trim range",
  trimPlanned: "Drag the timeline handles, then generate a local preview.",
  trimStart: "Start time",
  trimEnd: "End time",
  trimStartHandle: "Trim start handle",
  trimEndHandle: "Trim end handle",
  trimTimeline: "Trim timeline",
  resetTrim: "Reset trim",
  speed: "Speed",
  speedDetail: "Preview and export playback speed.",
  playbackSpeed: "Playback speed",
  resetSpeed: "Reset speed",
  manualSubtitles: "Manual subtitles",
  subtitleDetail: "Add simple timed cues for this clip.",
  addSubtitleCue: "Add subtitle",
  subtitleText: "Subtitle text",
  subtitleStart: "Subtitle start",
  subtitleEnd: "Subtitle end",
  removeSubtitleCue: "Remove subtitle",
  noSubtitles: "No subtitles yet",
  subtitleTimeline: "Subtitle timeline",
  resetSubtitles: "Reset subtitles",
  format: "Format",
  formatConversion: "Format conversion",
  imageFormat: "Image format",
  imageFormatHelper: "This format does not use a quality value. Export will generate it locally.",
  imageQualityHelper: "Quality affects lossy formats such as JPEG, WebP, and AVIF.",
  generateImageFormatPreview: "Generate format preview",
  generatingImagePreview: "Generating image preview...",
  imageFormatPreviewTask: "Format preview - {format}",
  imagePreviewReady: "Image preview ready.",
  videoFormat: "Video format",
  resetFormat: "Reset format",
  sourcePreview: "Source preview",
  derivedPreview: "Derived preview",
  composition: "Composition",
  playPreview: "Play preview",
  pausePreview: "Pause preview",
  resetTime: "Reset time",
  loopPlayback: "Loop",
  videoScrubber: "Video scrubber",
  generateVideoPreview: "Generate preview",
  generatingVideoPreview: "Generating preview...",
  videoFormatPreviewTask: "Format conversion - {format}",
  cancelVideoPreview: "Cancel",
  retryVideoPreview: "Retry preview",
  videoPreviewReady: "Preview ready.",
  videoPreviewFailed: "Preview failed.",
  videoPreviewCanceled: "Preview canceled.",
  previewStale: "Preview stale.",
  quality: "Quality",
  prepareExport: "Export",
  exportCurrentAsset: "Export asset",
  preparingImageExport: "Preparing local image export...",
  imageExportTask: "Export - {format}",
  downloadReady: "Export ready.",
  exportSaved: "Export saved.",
  exportCanceled: "Export canceled.",
  cancelExport: "Cancel export",
  retryExport: "Retry export",
  videoExportNext: "Preparing local video export...",
  videoExportTask: "Export - {format}",
  videoExportHelper: "Video export runs locally with ffmpeg.wasm. Large clips may take time.",
  largeVideoWarning:
    "Large local video job. Browser ffmpeg may take longer or fail on memory or codec limits.",
  videoPreviewCompatibilityWarning:
    "{format} can be downloaded, but may not play in the browser preview. MP4 or WebM is safest for generated previews.",
  imageExportFailed: "Image export failed.",
  imageExportFormatUnsupported: "This browser cannot export the selected image format.",
  canvasUnavailable: "Canvas export is not available in this browser.",
  imageLoadFailed: "Image preview could not be loaded for export.",
  canvasExportFailed: "Canvas export failed.",
  imageExportHelper: "Image export runs in this browser and does not upload media.",
  download: "Download",
  previewOf: "Preview of",
  unsupportedFormat: "Unsupported Format",
  selectedAsset: "Selected asset",
  mediaInfo: "Media info",
  previewBackground: "Preview background",
  transparentBackground: "Transparent",
  blackBackground: "Black",
  fileName: "File name",
  fileSize: "File size",
  dimensions: "Dimensions",
  duration: "Duration",
  mimeType: "MIME type",
  notAvailable: "Not available",
  transformTab: "Transform",
  adjustmentsTab: "Beautify",
  layersTab: "Layers",
  backgroundTab: "Background",
  trimTab: "Trim",
  speedTab: "Speed",
  subtitlesTab: "Subtitles",
  formatTab: "Format",
  scrollTabsLeft: "Scroll tabs left",
  scrollTabsRight: "Scroll tabs right",
} as const;
