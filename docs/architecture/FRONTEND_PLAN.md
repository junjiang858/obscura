...
- Interaction model:
  ...
  - Processing Center badge semantics follow attention priority instead of total history count: failed jobs show the highest-priority failed count in red, active queued/loading/processing jobs show the active count in accent blue, and newly completed unacknowledged jobs show a green result count. If only acknowledged completed or canceled history remains, the center stays available but the badge is hidden.
  - Opening the Processing Center marks newly completed jobs as acknowledged, which removes the green result badge without deleting task history. Failed jobs remain attention-worthy until the user clears them or a future retry flow succeeds. Completed/canceled jobs may be cleared individually or through a clear-completed action; failed jobs are not removed by clear-completed.
